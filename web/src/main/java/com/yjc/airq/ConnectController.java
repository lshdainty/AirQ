...
public class ConnectController {
	...
	public JSONObject selectTender(HttpServletRequest request) {
		...
		ArrayList<TenderVO> tenderList = connectService.selectTender(sort, member_id, criteria.getStartnum(),
				criteria.getEndnum());
	...
	@ResponseBody
	public void tenderList(MultipartFile[] uploadFile, TenderVO tenderVo, UploadVO uploadVo,
		...
		for (MultipartFile multipartFile : uploadFile) {
			...
			tenderVo.setUpload_code(upload_code);
	...
	@ResponseBody
	public JSONObject tenderContent(@PathVariable String tender_code, BidVO bidVo, UploadVO uploadVo, HttpServletRequest request) {
		String member_id = ((MemberVO) request.getSession().getAttribute("user")).getMember_id();
		...
		MemberVO memberVo = mypageService.memberInfo(tenderVo.getMember_id());
		// 투찰 리스트
	...
	public ResponseEntity<Resource> downloadFile(@RequestHeader("User-Agent") String userAgent, String fileName,
		...
		Resource resource = new FileSystemResource(
				request.getServletContext().getRealPath("/resources/uploadFile/ppt/") + fileName);
		...
		try {
			...
			headers.add("Content-Disposition",
					"attachment; filename=" + new String(resourceName.getBytes("UTF-8"), "ISO-8859-1").substring(36));
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
	...
	@ResponseBody
	public void addBidComplete(@PathVariable String tender_code, MultipartFile[] uploadFile, String sBid_price,
			UploadVO uploadVo, BidVO bidVo, HttpServletRequest request, Model model) {
	...
	public JSONObject selectCompare(Model model, HttpServletRequest request) {
		...
		if (sido.equals("광역시/도") && sigoon.equals("선택") && space == 0 && matter.equals("측정 물질")) {
			...
			pList = connectService.selectList(sido, sigoon, space, matter, sort, criteria.getStartnum(),
					criteria.getEndnum());
		}
	...
	public String productInsert(Model model, HttpServletRequest request, MultipartFile[] thumbnail) {
		...
		productVO.setCompany_code(
				connectService.company_code(((MemberVO) request.getSession().getAttribute("user")).getMember_id()));
	...
	public String productUpdate(Model model, HttpServletRequest request,
			@RequestParam("product_code") String product_code, MultipartFile[] thumbnail, String ori_thumbnail) {
		ProductVO productVO = new ProductVO();
		UploadVO uploadVO = new UploadVO();
