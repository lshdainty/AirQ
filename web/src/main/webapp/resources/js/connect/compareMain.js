...
function typeCheck(){
	var typeCheck = $(document).find(".on").attr("id");
	return typeCheck
}
...
function listAjax(data){
	...
	if(data.pList.length==0){
		...
		$(".compare-wrap").children().eq(3).removeClass("compare-list_grid");
		$(".compare-wrap").children().eq(3).removeClass("compare-list");
		$(".compare-wrap").children().eq(3).addClass("compare-list");
		$(".compare-list").empty();
...
function thumbAjax(data){
	...
	if(data.pList.length==0){
		...
		for(var i=0; i<data.pList.length; i++){
			...
			result += '<div class="compare-post_grid post-item" id="'+data.pList[i].product_code+'">';
			result += '<div class="compare-thumb_grid"><img src="'+"/resources/uploadFile/images/"+data.pList[i].file_name+'" alt="이미지X"></div>'
			result += '<div class="compare-info_grid"><div class="compare__title"><span>'+data.pList[i].product_name+'</span></div>'
			result += '<div class="compare__content">측정 적절 평수 : <span>'+space+'</span><br/>';
			result += '측정 지점 : <span>'+data.pList[i].measure_point+'</span><br/>';
			result += '만족도 평균 : <span>'+data.pList[i].staravg+'</span><br/>';
			result += '판매 건수 : <span>'+data.pList[i].sellnum+'</span><br/>';
		...
		$(".compare-wrap").children().eq(3).removeClass("compare-list_grid");
		$(".compare-wrap").children().eq(3).removeClass("compare-list");
		$(".compare-wrap").children().eq(3).addClass("compare-list_grid");
		$(".compare-list_grid").empty();
		$(".compare-list_grid").append(result);
		if(data.criteria.prev){
			page += '<li class="page-item"><a class="page-link" href="javascript:page('+(data.criteria.startPage-1)+');" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>'
		}
		for(var i=data.criteria.startPage; i<=data.criteria.endPage; i++){
...
$(document).on('click','.compare-post_grid',function(){
	var product_code = $(this).attr("id");
	window.location.href="/product?product_code="+product_code;
});
//상품 상세페이지 끝
